...
class DirectoryListing
    ...
    if list_address?(record, hkey)
      ...
      akeys.each do | atype |
        ...
        if addr.nil_or_empty?
          street = record["#{addr_prefix}street"] 
          city   = record["#{addr_prefix}city"] 
          state  = record["#{addr_prefix}state"] 
          zip    = record["#{addr_prefix}zip"]
  ...
  def add_phones(record, key)
    ...
    [ 
      ...
      [ PVWKEY_MH, list_phone?(record, hkey, mkey, 'ph_home_'), "#{mpre}home_phone"],
      [ PVWKEY_MW, list_phone?(record, hkey, mkey, 'ph_work_'), "#{mpre}work_phone"],
      [ PVWKEY_MC, list_phone?(record, hkey, mkey, 'ph_mobile_'), "#{mpre}cell"],
      [ PVWKEY_MP, list_phone?(record, hkey, mkey, 'ph_pager_'), "#{mpre}pager"],
      [ PVWKEY_FH, list_phone?(record, hkey, fkey, 'ph_home_'), "#{fpre}home_phone"],
      [ PVWKEY_FW, list_phone?(record, hkey, fkey, 'ph_work_'), "#{fpre}work_phone"],
      [ PVWKEY_FC, list_phone?(record, hkey, fkey, 'ph_mobile_'), "#{fpre}cell"],
      [ PVWKEY_FP, list_phone?(record, hkey, fkey, 'ph_pager_'), "#{fpre}pager"],
    ].each do | tuple |
  ...
  def add_emails(record, key)  
    ...
    [ 
      [ PVWKEY_ME1, list_email?(record, hkey, mkey, 'em1_'), "#{mother_prefix}email"],
      [ PVWKEY_ME2, list_email?(record, hkey, mkey, 'em2_'), "#{mother_prefix}email2"],
      [ PVWKEY_FE2, list_email?(record, hkey, fkey, 'em1_'), "#{father_prefix}email"],
  ...
  class << self
    ...
    def add_students_with_home_id(home_id, other_students)
      ...
      for student in students
        hkey = student['home2_id'] == home_id ? 'h2_' : 'h1_'
        if @debug || list_student?(student, hkey)
          lname = student['last_name'] 
          fname = student['nickname'] 
          if fname == ''
            fname = student['first_name']
          end 
          student_number = student['student_number'] 
          key = "#{lname}:#{fname}:#{student_number}".upcase 
          new_students[key] = student.attributes
        end
      end
      new_students
    end
