...
class FPDF
  ...
  def initialize(orientation='P', unit='mm', format='A4', fontpath='')
    ...
    @pages=[]
    @OrientationChanges=[]
    @PageRotations=[] # PFZ rotated pages
    @state=0
    @fonts={}
    @FontFiles={}
    @diffs=[]
  ...
  def AddPage(orientation='', rotation=0)
    # Start a new page
    ...
    # Start new page
    beginpage(orientation, rotation)
  ...
  def AddFont(family, style='', file='')
    ...
    if !@fontpath.empty? then
      if @fontpath[-1,1] == '/' then
        file = @fontpath + file
      else
        file = @fontpath + '/' + file
      end
    ...
    if true
      # original source had a require statement
      load(file)
      ...
      i = @fonts.length + 1
      #print "loaded #{file} for /F#{i}, key is #{fontkey}\n"
      #print " FontDef.name is #{FontDef.name}\n"
      #print " FontDef.file is #{FontDef.file}\n"
  ...
  def RotatedText(rot, x, y, txt)
    # Output a string, with a rotation matrix
    a = 1.0 
    b = 0.0
    c = 0.0
    d = 1.0
    if rot == 90
      ...
      a = Math.cos(Math::PI*rot/180.0)
      b = Math.sin(Math::PI*rot/180.0)
      c = -b
      d = a
    end
    # tx, ty are in real space (origin at lower left)
    tx = x*@k
    ty = (@h-y)*@k
    # translate, then rotate, then translate back
    e = tx - a*tx - c*ty
    f = ty - b*tx - d*ty
    txt.gsub!(')', '\\)')
    txt.gsub!('(', '\\(')
    txt.gsub!('\\', '\\\\')
    s=sprintf('BT %.2f %.2f %.2f %.2f %.2f %.2f Tm %.2f %.2f Td (%s) Tj ET',
      a,b,c,d,e,f,tx,ty,txt);
    s=s+' '+dounderline(x,y,txt) if @underline and txt!=''
    s=@TextColor+' '+s if @ColorFlag
    s='q '+s+' Q'
    out(s)
  end
  ...
  def putpages
    ...
    1.upto(nb) do |n|
      ...
      out('<</Type /Page')
      out('/Parent 1 0 R')
        out(sprintf('/Rotate %d', @PageRotations[n]))
  ...
  def beginpage(orientation, rotation)
    @page=@page+1
    @pages[@page]=''
    @state=2
    @x=@lMargin
